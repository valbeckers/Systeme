...
function cleanSystemState(raw){
  ...
  return {
    ...
    inventory:{
      ...
      debtRecognition:Math.max(0,Math.floor(Number(data.inventory&&data.inventory.debtRecognition)||0))
    },
    ...
    debtRecognitionMilestones:Array.isArray(data.debtRecognitionMilestones)?data.debtRecognitionMilestones:[],
...
function buildState(){
  ...
  const out={
    ...
    inventory:saved.inventory||{majorElixir:0,minorElixir:0,supremeElixir:0,transmutationGrimoire:0,masterContract:0,overachievementMark:0,recordHammer:0,teleportCrystal:0,invisibilityCape:0,debtRecognition:0},
    ...
    debtRecognitionMilestones:saved.debtRecognitionMilestones||[],
...
function App(){
  ...
  function tryDungeonItemDrops(dungeonId){
    awardInventoryItem("minorElixir","guaranteed");
    [["majorElixir",.10],["supremeElixir",.01],["masterContract",.10],["overachievementMark",.10],["recordHammer",.10],["teleportCrystal",.10],["invisibilityCape",.10]].forEach(([id,p])=>{if(Math.random()<p)awardInventoryItem(id,"rare");});
    if(dungeonId==="alchemist" && Math.random()<0.25) awardInventoryItem("transmutationGrimoire","rare");
  }
  ...
  useEffect(()=>{
    ...
    setState(s=>{
      ...
      if(s.streakBonusDay!==t){
        ...
        const debtMilestones=next.debtRecognitionMilestones||[];
        if(newStreak>0 && newStreak%5===0 && !debtMilestones.includes(newStreak)){
          next={...next,debtRecognitionMilestones:[...debtMilestones,newStreak]};
          setTimeout(()=>awardInventoryItem("debtRecognition","guaranteed"),350);
        }
        let streakAnim = {
          title:"STREAK BONUS !",
          streak:newStreak,
  ...
  function createQuestDebt(obj){
    ...
    setState(s=>{
      ...
      if((Number(inv.debtRecognition)||0)<1) return s;
      ...
      inv.debtRecognition=Math.max(0,(Number(inv.debtRecognition)||0)-1);
      return {
        ...s,
        inventory:inv,
        questDebt:{
...
const BONUS_BADGE_COLOR = "#fbbf24";
    ...
    return h("div",{class:"qi "+(d>=effectiveT&&effectiveT>0?"done":""),style:marked?"border-color:#a78bfa;background:rgba(124,58,237,.14);box-shadow:0 0 16px rgba(139,92,246,.42);animation:pulse 1.8s ease-in-out infinite":""},
      ...
      (!obj.optional&&!obj.weekly&&isDebtEligibleQuest(obj)&&d<effectiveT)&&h("div",{style:"margin-top:8px"},
        state.questDebt&&state.questDebt.status==="active"&&state.questDebt.id===obj.id
          ...
          : (!state.questDebt||state.questDebt.status!=="active")&&((state.debtUsesByWeek||{})[wk]||0)<MAX_DEBTS_PER_WEEK&&Math.max(0,Math.floor(Number(state.inventory&&state.inventory.debtRecognition)||0))>0
            ? h("button",{
            ...
            : (!state.questDebt||state.questDebt.status!=="active")&&Math.max(0,Math.floor(Number(state.inventory&&state.inventory.debtRecognition)||0))<1
              ? h("div",{style:"font-family:Orbitron,sans-serif;font-size:8.5px;color:var(--td);letter-spacing:.6px;text-transform:uppercase"},"Reconnaissance de dette requise")
              : null
      ),
      (()=>{
  ...
  const INVENTORY_ITEMS={
    ...
    majorElixir:{name:"ÉLIXIR D’EXPÉRIENCE MAJEUR",short:"ÉLIXIR MAJEUR",emoji:"🧪",action:"CONSOMMER",pct:.20,desc:"Cet élixir vous permet de gagner 20 % d’XP en plus dans la statistique de votre choix pendant 24 h. Utilisez-le à bon escient !",obtain:["Après avoir complété un donjon (Taux : 10 %).","Après avoir complété toutes les quêtes journalières (Taux : 0,5 %).","Après avoir complété toutes les quêtes bonus (Taux : 0,5 %).","Après avoir complété une quête urgente (Taux : 0,5 %).","Après avoir accompli un nouveau record (Taux : 0,5 %)."]},
    minorElixir:{name:"ÉLIXIR D’EXPÉRIENCE MINEUR",short:"ÉLIXIR MINEUR",emoji:"🧪",action:"CONSOMMER",pct:.10,desc:"Cet élixir vous permet de gagner 10 % d’XP en plus dans la statistique de votre choix pendant 24 h. Utilisez-le à bon escient !",obtain:["Après avoir complété un donjon (Taux : 100 %).","Après avoir complété toutes les quêtes journalières (Taux : 1 %).","Après avoir complété toutes les quêtes bonus (Taux : 1 %).","Après avoir complété une quête urgente (Taux : 1 %).","Après avoir accompli un nouveau record (Taux : 1 %)."]},
    supremeElixir:{name:"ÉLIXIR D’EXPÉRIENCE MAGISTRAL",short:"ÉLIXIR MAGISTRAL",emoji:"🧪",action:"CONSOMMER",pct:.30,desc:"Cet élixir vous permet de gagner 30 % d’XP en plus pendant 24 h. Utilisez-le à bon escient !",obtain:["En fusionnant 5 Élixirs d’expérience mineurs via le Grimoire de transmutation (Taux : 100 %).","Après avoir complété un donjon (Taux : 1 %).","Après avoir complété toutes les quêtes journalières (Taux : 0,1 %).","Après avoir complété toutes les quêtes bonus (Taux : 0,1 %).","Après avoir complété une quête urgente (Taux : 0,1 %).","Après avoir accompli un nouveau record (Taux : 0,1 %)."]},
    transmutationGrimoire:{name:"GRIMOIRE DE TRANSMUTATION",short:"GRIMOIRE",emoji:"📔",action:"TRANSMUTER",desc:"Permet de fusionner cinq Élixirs mineurs pour créer un Élixir magistral.",obtain:["Après avoir complété un Donjon de l’Alchimiste (Taux : 25 %).","Après avoir complété toutes les quêtes journalières (Taux : 1 %).","Après avoir complété toutes les quêtes bonus (Taux : 1 %).","Après avoir complété une quête urgente (Taux : 1 %).","Après avoir accompli un nouveau record (Taux : 1 %)."]},
    ...
    debtRecognition:{name:"RECONNAISSANCE DE DETTE",short:"RECONNAISSANCE DE DETTE",emoji:"🧾",action:"CONSERVER",desc:"Permet de reporter le reliquat d’une quête journalière éligible et de le rembourser dans le délai prévu par le système de dette.",obtain:["Après avoir atteint un palier de 5 Streak (Taux : 100 %).","Après avoir complété toutes les quêtes journalières (Taux : 1 %).","Après avoir complété toutes les quêtes bonus (Taux : 1 %).","Après avoir complété une quête urgente (Taux : 1 %).","Après avoir accompli un nouveau record (Taux : 1 %)."]}
  };
  function itemQty(id){ return id==="dungeonKey"?dungeonKeys:Math.max(0,Math.floor(Number(state.inventory&&state.inventory[id])||0)); }
  function Inventory(){
    const ids=["dungeonKey","majorElixir","minorElixir","supremeElixir","transmutationGrimoire","masterContract","overachievementMark","recordHammer","teleportCrystal","invisibilityCape","debtRecognition"]
      .sort((a,b)=>{
  ...
  function InventoryItemModal(){
    ...
    let disabled=qty<1 || id==="debtRecognition";
    let reason=qty<1?"Aucun exemplaire disponible.":(id==="debtRecognition"?"Cet objet est consommé automatiquement lors de la création d’une dette.":"");
    if(id==="dungeonKey"){
      ...
      if(minors<5){disabled=true;reason="Cinq Élixirs mineurs sont nécessaires ("+minors+"/5).";}
      ...
      h("div",{style:"display:flex;gap:10px;margin-top:22px"},
        ...
        h("button",{class:"rudis",style:"min-width:110px",onClick:()=>{
          ...
          if(id==="dungeonKey"){
            ...
            setState(s=>{const inv={...(s.inventory||{})};if((Number(inv.minorElixir)||0)<5||(Number(inv.transmutationGrimoire)||0)<1)return s;inv.minorElixir=Math.max(0,(Number(inv.minorElixir)||0)-5);inv.transmutationGrimoire=Math.max(0,(Number(inv.transmutationGrimoire)||0)-1);inv.supremeElixir=Math.max(0,(Number(inv.supremeElixir)||0)+1);return {...s,inventory:inv};});
  ...
  function ItemUseUp(){
    ...
    return h("div",{class:"ruov",style:"--rc:"+rank.color+";--rg:"+rank.glow},h("div",{class:"rucont"},
      ...
      itemUseUp.transmuted&&h("div",{class:"rulabel",style:"margin-top:12px;max-width:330px;line-height:1.5;color:#c084fc"},"5 Élixirs mineurs ont été fusionnés en 1 Élixir magistral."),
