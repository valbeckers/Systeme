...
} from "./breachEngine.js?v=20260821-alduin-rupture-v1";
import {
  ELIXIR_STATS,
  ELIXIR_DURATION_MS,
  isElixirKind,
...
function App(){
  ...
  const questXpEarnedToday=(obj,amount,target)=>{
    ...
    if(obj.weekly&&obj.completionBonusXp){
      ...
      if(prior<target&&prior+(Number(amount)||0)>=target)earned+=Number(obj.completionBonusXp)||0;
    }
    return earned;
  };
  ...
  function validate(obj,e,forceVal){
    ...
    if(obj.completionBonusXp&&prev<b&&next>=b)xp+=Number(obj.completionBonusXp)||0;
    setState(s=>{
      let next2 = s;
      if(obj.weekly){
        const w={...s.weeklyLog};w[wk]={...(w[wk]||{}),[obj.id]:(w[wk]?.[obj.id]||0)+val};
